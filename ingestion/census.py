...
Run locally:
    python ingestion/ingest_census.py
...
# This is how the Census API identifies NYC boroughs
...
CREATE TABLE IF NOT EXISTS URBANPULSE.RAW.census_raw (
    ...
    -- Derived (calculated in ingestion, not dbt)
    -- We calculate poverty rate here as a raw field
    -- dbt will validate and potentially recalculate this
...
def fetch_census_data() -> list[dict]:
    ...
    """
    # Build the variables string for the API call
    variables = "NAME," + ",".join(CENSUS_VARIABLES.keys())

    params = {
        "get":     variables,
        "for":     "county:061,005,047,081,085",  # All 5 NYC counties
    ...
    # Census API returns a list of lists
    ...
    records = []
    for row in rows:
        # Map list values to header keys
        ...
        # Census API returns strings — convert to float, handle -1 (missing data)
        def safe_float(val):
            try:
                f = float(val)
                return None if f < 0 else f   # Census uses -1 for missing
        ...
        # Calculate poverty rate — handle division by zero
        ...
        record = {
            ...
            "median_household_income":  safe_float(data.get("B19013_001E")),
        ...
        log.info(
            ...
            f"median_income=${safe_float(data.get('B19013_001E')):,.0f}, "
            f"poverty={poverty_rate}%"
            if total_pop and safe_float(data.get("B19013_001E")) and poverty_rate
...
def load_to_snowflake(records: list[dict], cursor) -> int:
    """
    Load census records using MERGE to handle re-runs gracefully.
    If we run this twice for the same year, it updates rather than duplicates.
    This is idempotent ingestion — safe to run multiple times.
    """
    rows = [
        (
            r["borough"],
    ...
    # MERGE ensures idempotency — same borough + year = update, not duplicate
    cursor.execute(f"""
    ...
    cursor.executemany(
        """
        INSERT INTO census_staging VALUES (
